package com.uniken.automation.controller;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.google.gson.Gson;
import com.uniken.automation.model.ExecutionModel;
import com.uniken.automation.responses.TestExecutionHistoryResponse;

/**
 * Servlet implementation class ExecutionHistoryController
 */
@WebServlet("/getExceutionHistory.htm")
public class ExecutionHistoryController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ExecutionHistoryController() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		TestExecutionHistoryResponse resp = new TestExecutionHistoryResponse();
		try
		{
			resp = new ExecutionModel().getExecutionHistoryForDashboard();
		}
		catch(Exception e)
		{
			e.printStackTrace();
			resp.setError_code(7);
			resp.setError_message(e.getMessage());
			resp.setResponse_code(1);
		}
		
		response.getWriter().write(new Gson().toJson(resp));
	}

}
